...
def cost_of_resources(event, resource_list,account_id):
    ...
    try:
        ...
        gauge = Gauge("IAM_USER_Resource_Cost_List", "IAM User Resource List And Cost",
           ...
           registry=registry)
        # Creating guage metrics for total services cost of IAM User in specific AWS Region   
        g_user_cost = Gauge("IAM_USER_Total_Services_Cost_List", "IAM User Total Services Cost List",
     ...
     # getting resource detail using Owner tag        
       ...
       if dict_len == 0:
          ...
          case_list.append(result_list)
            ...
            result_list = {'User': user, 'ResourceList': subset_list, 'Region': regionName}
            case_list.append(result_list)
