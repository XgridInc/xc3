...
def cost_of_instance(event, client, resource_id, start_date, end_date):
    """
    Fetches the cost and usage information for a given resource ID within a specified time period.

    Args:
        client: The boto3 client to use for the AWS Cost Explorer API.
        resource_id (str): The ID of the resource to fetch cost information for.
        start_date (str): The start date of the time period to fetch cost information for, in 'YYYY-MM-DD' format.
        end_date (str): The end date of the time period to fetch cost information for, in 'YYYY-MM-DD' format.
...
def lambda_handler(event, context):
    ...
    Returns:
        A dictionary containing a status code and a message indicating that the function has completed.
    """
                    ...
                    if state != "terminated":
                        if state == "running":
                            ...
                            for j in range(len(response["ResultsByTime"])):
                                ...
                                iam_service_gauge.labels(
                                    (
                                        datetime.strptime(new_time, "%Y-%m-%dT%H:%M:%SZ")
